...
// p t elementtype dim
void readInput(string filename, vector<Int> &param, vector<Int> &t, vector<Int> &elementtype, vector<double> &p)
{
    // Open file to read
    ...
    if (in) {
        ...
        N = nvemax*ne;
        readiarrayfromdouble(in, t, N);
        N = ne;
        readiarrayfromdouble(in, elementtype, N);        
        N = np*dim;
        readdarray(in, p, N);                
    }
