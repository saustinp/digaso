void fhat_ehd_tof3d(double *fh, double *fh_udg, double *fh_uh, double *pg, double *udg, double *uh, double *nl, double *param, double time, int ng, int nc, int ncu, int nd, int ncd)
{
	...
	for (int i = 0; i <ng; i++) {
		...
		fh_uh[0*ng+i] = t0;		// Why was it 0?

	}
}

void fhatonly_ehd_tof3d(double *fh, double *pg, double *udg, double *uh, double *nl, double *param, double time, int ng, int nc, int ncu, int nd, int ncd)
{
	double param1 = param[0];
	double param2 = param[1];
	double param3 = param[2];
	...
	for (int i = 0; i <ng; i++) {
		...
		double t3 = 1.0/param5;
		double t0 = nl2*x1*(u1+param1*t2*t3*u3)+param10*x1*(u1-uh1)+nl1*param1*t2*t3*u2*x1;
		fh[0*ng+i] = t0;	// Why was this line not present before?
