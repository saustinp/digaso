void fb_streamer2d(double *fh, double *fh_udg, double *fh_uh, double *pg, double *udg, double *uh, double *nl, double *param, double time, int ng, int nc, int ncu, int nd, int ncd)
{
	...
	for (int i = 0; i <ng; i++) {
		...
		fh[0*ng+i] = param11*(u1-uh1)-nl1*(t2*t8*u6*uh1*2.3987-t2*t9*t10*t11*u4*4.3628E-3)-nl2*(t2*t8*u9*uh1*2.3987-t2*t9*t10*t11*u7*4.3628E-3);
		fh[1*ng+i] = param11*(u2-uh2);
		fh[2*ng+i] = nl1*u6+nl2*u9+param11*(u3-uh3);
	...
	for (int i = 0; i <ng; i++) {
		...
		fh_udg[9*ng+i] = nl1*t2*t3*t12*t13*4.3628E-3;
		fh_udg[10*ng+i] = 0.0;
		fh_udg[11*ng+i] = 0.0;
		fh_udg[12*ng+i] = 0.0;
		fh_udg[13*ng+i] = 0.0;
		fh_udg[14*ng+i] = 0.0;
		fh_udg[15*ng+i] = nl2*(t14+t2*t3*t9*t10*u6*u7*9.59816E-4)+nl1*(t3*t15*uh1*(-2.3987)+param3*t3*t4*t9*t11*uh1*6.23662E-1+t2*t3*t9*t10*u4*u6*9.59816E-4);
		fh_udg[16*ng+i] = 0.0;
		fh_udg[17*ng+i] = nl1;
		fh_udg[18*ng+i] = nl2*t2*t3*t12*t13*4.3628E-3;
		fh_udg[19*ng+i] = 0.0;
		fh_udg[20*ng+i] = 0.0;
		fh_udg[21*ng+i] = 0.0;
		fh_udg[22*ng+i] = 0.0;
		fh_udg[23*ng+i] = 0.0;
		fh_udg[24*ng+i] = nl1*(t14+t2*t3*t9*t10*u4*u9*9.59816E-4)+nl2*(t3*t15*uh1*(-2.3987)+param3*t3*t5*t9*t11*uh1*6.23662E-1+t2*t3*t9*t10*u7*u9*9.59816E-4);
		fh_udg[25*ng+i] = 0.0;
		fh_udg[26*ng+i] = nl2;
	...
	for (int i = 0; i <ng; i++) {
		...
		double t7 = param3*t6;
		double t8 = 1.0/pow(t7,1.3E1/5.0E1);
		fh_uh[0*ng+i] = -param11-nl1*t2*t8*u6*2.3987-nl2*t2*t8*u9*2.3987;
		fh_uh[1*ng+i] = 0.0;
		fh_uh[2*ng+i] = 0.0;
		fh_uh[3*ng+i] = 0.0;
...
void fbonly_streamer2d(double *fh, double *pg, double *udg, double *uh, double *nl, double *param, double time, int ng, int nc, int ncu, int nd, int ncd)
{
	double param1 = param[0];
	double param2 = param[1];
	double param3 = param[2];
