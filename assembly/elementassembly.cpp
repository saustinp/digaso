        ...
        Int ie, int computeJacobian)
{
    ...
    double *shapvt, *s, *stg, *f, *s_udg, *f_udg, *pg, *udg, *odg, *udgg, *odgg, *sh;
    shapvt = &master.shapvt[0];
    s = &temp.s[0];
    stg = &temp.stg[0];
    f = &temp.f[0];
...
//                     &npv, &one, s, &ngv);
        ...
        if (app.axisymmetry==1) {
            ...
            for (j=0; j<ncu; j++) {
                for (i=0; i<ngv; i++) {
                    s[j*ngv+i] += (stg[j*ngv+i] - udgg[j*ngv+i]*app.fc_u)*pg[i]*app.fcu_vector[j];     // Axisymmetric, *r
            ...
            // s = s + app.fc_u*(stg - udgg)
            for (j=0; j<ncu; j++) {
                for (i=0; i<ngv; i++) {
                    s[j*ngv+i] += (stg[j*ngv+i]-udgg[j*ngv+i]*app.fc_u)*app.fcu_vector[j];
                    if (computeJacobian == 1)
