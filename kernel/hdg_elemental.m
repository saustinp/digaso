    ...
    %     pause
        ...
        %   DinvRu  DinvF     H  Rh in digaso
            if isfield(app,'debug_digaso')
                DinvRu = readbin('./debug/debug27.bin');   % Compare with dudg: first 3 columns: tmp=dudg(:,1:ncu,:); max(tmp(:)-DinvRu)=1.2385. First element matches though
                DinvF = readbin('./debug/debug28.bin');     % tmp=dudg_duh(:,1:ncu,:,:,1); max(tmp(:)-DinvF(1:486))=68.1129. Upon closer inspection, the signs are reversed, so doing max(tmp(:)+DinvF(1:486))=1.8771e-08. Max across the entire array is 83.6884
                H = readbin('./debug/debug25.bin');        % First two elements match, but the max error across the entire array (11765331 entries) is 9.3384
                Rh = readbin('./debug/debug26.bin');      % First two elements match, but the max error across the entire array is 186.4416
