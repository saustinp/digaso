...
//             #pragma omp for
    ...
    case 2:
        ...
        for (int ri = 0; ri < sys.entpartpts[0]; ri++) {
            ...
            for (int i=0; i<nri; i++) {
//                 __builtin_prefetch(&x_sp[5 * (*ent2ent_pos + 50)],0,3);
//                 __builtin_prefetch(Hgpos+25*5,0,3);
...
//             #pragma omp for
    ...
    case 2:
        ...
        for (int ri = sys.entpartpts[0]; ri < sys.BJ_nrows; ri++) {
            ...
            for (int i=0; i<nri; i++) {
//                 __builtin_prefetch(&x_sp[5 * (*ent2ent_pos + 5)],0,3);
...
//             #pragma omp for
            for (Int ri = 0; ri < sys.BJ_nrows; ri++) {
                ...
                nriInt = i;
                if (nriInt == 0)
                    error("Error 4FT6H8 in gmresmpi.cpp was detected.\n");
...
// static void matvecMPI_BJ(double* v, sysstruct &sys, double* x, Int * requestCounter, double* matvectimes)
// {
//     if (sys.matvecPrecision == 0 && sys.robustMode == 0)
//         matvecMPI_BJ_sp(v, sys, x, requestCounter, &matvectimes[0]);
//     else
//         matvecMPI_BJ_dp(v, sys, x, requestCounter, &matvectimes[0]);
// }
...
//         matvecMPI_BJ(v, sys, x, requestCounter, &matvectimes[0]);
