...
void applyMPIpreconditioner(sysstruct &sys, double *r, Int subdomainPreconditioner, Int sendInputVector, Int sendOutputVector, Int *requestCounter, double *precondtimes)
{
    clock_t t;
    
    // Send input vector. Only required for RAS(l), l>0 preconditioner
    t = clock();
    if (sys.parallelPreconditioner == ?? && sendInputVector == 1)
        sendvector(sys, &r[0], requestCounter);
    precondtimes[0] += clock() - t;

    // Apply preconditioner:
    t = clock();
    applyPreconditioner(sys, &r[0], subdomainPreconditioner, requestCounter);
    ...
    *convergenceFlag = 0;
    ...
    if (orthogMethod == 0) {        /* MGS method */
        ...
        * iterPerformedPt = iter;
        ...
        if (my_rank == 0)
            printf("IMGS not implemented in MPI code yet.\n");
        error("\n");
    }
...
void gmresmpi(sysstruct &sys, Int *convFlag)
{
    ...
    Int N = sys.blkStartR[sys.BJ_nrows];
    Int preconditionerSide = sys.preconditionerSide;
    Int subdomainPreconditioner = sys.subdomainPreconditioner;
    Int parallelPreconditioner = sys.parallelPreconditioner;
    ...
    if (preconditionerSide == 0) {
        ...
        sendInputVector = 1;
        sendOutputVector = 0;
        applyMPIpreconditioner(sys, &sys.Rg[0], subdomainPreconditioner, sendInputVector, sendOutputVector, requestCounter, &precondtimes[0]);     // TODO: Need to allocate more memory for Rg (??)
        BILU0time += clock() - t;
    }
    ...
    while (iter < sys.maxiter) {        
        if (iter == 0) {
            ...
            if (preconditionerSide == 0) {
                ...
                if (parallelPreconditioner == ??)
                    sendOutputVector = 1;
                else if (parallelPreconditioner == ?? || parallelPreconditioner == ?? || parallelPreconditioner == ??)
                    sendOutputVector = 0;
                ...
                sendInputVector = 0;
                sendOutputVector = 0;
                applyMPIpreconditioner(sys, &sys.Mx[0], subdomainPreconditioner, sendInputVector, sendOutputVector, requestCounter, &precondtimes[0]);
                BILU0time += clock() - t;
    ...
    if (preconditionerSide == 1) {
        ...
        sendInputVector = 1;       // sys.x was just computed (no intermediate operations before reaching this line)
        sendOutputVector = 0;
        applyMPIpreconditioner(sys, &sys.x[0], subdomainPreconditioner, sendInputVector, sendOutputVector, requestCounter, &precondtimes[0]);
        BILU0time += clock() - t;
    }
    ...
    if (sys.print >= 2 && sys.my_rank == 0) {
        ...
        printf("Matrix-vector product performance: %g GBytes/s \n", (bytesPerNumber_matvec * iter * (long) sys.rowStartJ[sys.BJ_nrows] / 1.0e9)/(matvectime/CLOCKS_PER_SEC));

        printf("\n2. Preconditioner solve time: %g ms\n", (BILU0time/CLOCKS_PER_SEC)*1.0e3);
        for (Int i = 0; i < 2; i++)
            printf("2.%d: %g ms\n", i, (precondtimes[i]/CLOCKS_PER_SEC)*1.0e3);
        
        if (subdomainPreconditioner != -1)
            printf("Preconditioner performance: %g GBytes/s \n", (bytesPerNumber_precSolve * iter * (long) sys.blkStartP[sys.numBlocksP] / 1.0e9)/(BILU0time/(CLOCKS_PER_SEC)));
