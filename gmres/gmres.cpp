...
void orthogonalize(double * c, double * q, double * Q, Int m, Int n, Int orthogMethod, 
...
void gmres(sysstruct &sys, Int *convFlag)
{
    ...
    while (iter < sys.maxiter) {
	    ...
	    //printf("SIZE OF VECTOR SYS.Y = %lu AND SYS.E1 = %lu AND rowsSLP = %d .\n", sys.y.size(),sys.e1.size(),rowsLSP);
            for (i=0; i<rowsLSP; i++)
                sys.y[i] = sys.e1[i];
