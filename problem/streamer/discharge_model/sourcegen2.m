...
ni_bar = odg(1); % 
ni_tilde = (ni_bar + se) / factor(1); % factor(1) = fc_u
...
for n=1:1
    ...
    matlabFunction(f(:),jac_f(:),'file','tmp.m','vars',{p,udg,odg,param,factor,time,[zero one]},'outputs', {'s','s_udg'});          % NOTE: This was changed from the template
