...
for i = 0:2:8
    load(['result' num2str(i) '.mat'])
    figure(1); clf; scaplot(mesh1,1e6*UDG(:,2,:)*eps0*E0/a,[0 64],1); 
    %figure(1); clf; scaplot(mesh1,1e3*UDG(:,2,:)*a/(eps0*E0),[0 64],1); 
    axis equal; axis off; colormap jet; colorbar('FontSize',15);    
    axis([-15 25 -20 20]*scale);
    set(gca, 'LooseInset', get(gca, 'TightInset'));
    fn = ['chargedensity_R55_theta30_wind' num2str(i)];
    print('-dpng',fn,'-r300');
end
...
for i = 0:2:8
    ...
    ds = loginc(linspace(0.1,1,700)*10,1);
    ...
    ylabel('y (cm)','FontSize',20);
    axis([-1 17 -6 8]);
    fn = ['ionstreamline_R55_theta30_wind' num2str(i)];
    print('-dpng',fn,'-r300');
end
...
for i = 0:2:8
    ...
    [Peek1,x1,y1,s1,Ex1,Ey1] = PeekIntegral(mesh1, 0, 1, 0*Ua, Ua, x0, y0, ds, 2, pva(:,1), pva(:,2));        
    figure(5);clf;     
    plot(etaw,Peek1+Rpeek-max(Peek1(:)),etaw,Rpeek*ones(size(etaw))); 
    set(gca,'FontSize',18);
    xlabel('\theta','FontSize',20);
    ylabel('Peak Integral','FontSize',20);
    axis([0 2*pi 3.6 4.2]);
    
    fn = ['peekintegral_R55_theta30_wind' num2str(i)];
    print('-dpng',fn,'-r300');
end
