...
vector <Int> sol.scalarFieldStart[ne+1];
vector <Int> mesh.bfStart[ne+1]; // cumsum(0, nfe's)

THIS FILE HASN'T BEEN COMPLETELY PORTED TO v1.0 (ONLY THE BEGINNING OF IT)
        ...
        tempstruct &temp, Int* ndims, Int bouID)
{
    ...
    Int *elconfullidx = &mesh.elconfullidx[0];
    
    Int nein;
    if (sys.nproc > 1)
        nein = sys.elempartpts[0] + sys.elempartpts[1];
    else
        nein = mesh.ne;//ndims[5];
    ...
    for (Int ie = 0; ie < nein; ie++) {
        ...
        Int nfe = mesh.elements[elementype].nfe;
        Int nc = ndims[19];
        Int ncu = ndims[20];
        Int nch = ndims[23];
        Int ndf = mesh.ndf[ie];
        
        Int *elcfull = &mesh.elconfull[elconfullidx[ie]];
        double *udg = &sol.UDG[sol.UDGstart[ie]];
        double *udg_ref = &sol.UDGi[sol.UDGstart[ie]];
        double *av = &sol.avField_p1CG[sol.scalarFieldStart[ie]];
        double *uh = &temp.uh[0];
        double *uh_ref = &temp.uh_ref[0];
        double *pn = &mesh.dgnodes[mesh.dgnodesidx[ie]];
        for (Int j=0; j<ndf; j++) {
            Int n = elcfull[j];
            for (Int k=0; k<nch; k++) {
...
//         faceflux(mesh, master, app, sol, temp, ndims, ie);
        ...
        bf = &mesh.bf[mesh.bfStart[ie]];
        
        for (Int is = 0; is < nfe; is++) {
            if (bf[is] == -bouID) {
                Int nqfR = master[elementype].nqfR[0]?? this is a vector;
                for (Int ig = 0; ig < nqfR; ig++)
                    for (Int id = 0; id < nd; id++) {
