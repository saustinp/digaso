...
//               app.precPrecision app.matvecPrecision app.orthogPrecision app.adaptiveGMREStol app.flag];
    ...
    sol.UDG2Write.resize(ne*npe*ncu);       // Only required if writeQflag = 1.
    ...
    sys.maxiter = app.problem[14];             // Maximum number of GMRES iterations
    sys.restart = app.problem[15];              // Parameter k in GMRES(k)
    sys.preconditioner = app.flag[16];         // 0: Restricted additive Schwarz (RAS) + BILU(k), 1: Subdomain-wise block Jacobi (BJ) + BILU(0), 2: Entity-wise block Jacobi (BJ)
    ...
    if (app.quasiNewton == 1) {
        ...
        if (sys.quasiNewtonAccuracy == 0) {
            ...
            error("sys.quasiNewtonAccuracy has invalid value in initializestructs.cpp\n");
    }
...
//     sol.UH_initMR.resize(nch*ndh);
    ...
    if ((sys.precPrecision == 0) && (sys.preconditioner == 0 || sys.preconditioner == 1) && (sys.reorderMethod == 3 || sys.reorderMethod == 4))
        error("MDF with constraints not implemented for BILU0 and BILUk in mixed-precision.\n");
    ...
    if (sys.preconditioner == -1) {       // No preconditioner
        sys.nrowsP = 0;
        sys.nblksP = 0;
    }
    else if (sys.preconditioner == 0) {      // RAS + BILU(k)
        block_crs_k(&sys.colindP, &sys.rowptsP, &sys.ent2ent, &sys.ent2entStart, sys.numEntities, sys.kLevel);
        sys.nrowsP = sys.rowptsP.size()-1;
        sys.nblksP = sys.colindP.size();
        computeMgMinusHg(sys);
    }
    else if (sys.preconditioner == 1) {      // BJ + BILU(k)
        block_crs_k(&sys.colindP, &sys.rowptsP, &sys.ent2ent, &sys.ent2entStart, sys.BJ_nrows, sys.kLevel);
        sys.nrowsP = sys.rowptsP.size()-1;
        sys.nblksP = sys.colindP.size();
        if (sys.nrowsP != sys.BJ_nrows)
            error("Number of rows in parallel BJ preconditioner is not correct.\n");
        computeMgMinusHg(sys);
    }
    else if (sys.preconditioner == 2) {     // Entity-based block Jacobi
        sys.nrowsP = sys.BJ_nrows;
        sys.nblksP = sys.BJ_nrows;
        sys.rowptsP.resize(sys.nrowsP+1);
        sys.colindP.resize(sys.nblksP);
        for (int cc=0; cc<sys.nrowsP; cc++) {
            sys.rowptsP[cc] = cc;
            sys.colindP[cc] = cc;
        }
        sys.rowptsP[sys.nrowsP] = sys.nrowsP;
    }
    else
        error("Preconditioner flag not recognized.\n");
    
//     printf("%d, %d, %d, %d\n",sys.numBlocks,sys.nblksP,sys.numEntities,sys.nrowsP);
    
    Int nn, maxBlocksPerRowP = 0, minBlocksPerRowP = 999999;
    for (int cc=0; cc<sys.nrowsP; cc++) {
        nn = sys.rowptsP[cc+1] - sys.rowptsP[cc];
        minBlocksPerRowP = (minBlocksPerRowP>nn) ? nn : minBlocksPerRowP;
        maxBlocksPerRowP = (maxBlocksPerRowP>nn) ? maxBlocksPerRowP : nn;
    }
    sys.maxBlocksPerRowP = maxBlocksPerRowP;
    sys.minBlocksPerRowP = minBlocksPerRowP;
    
    // Note: The following sizes are more than required for BJ preconditioner
    sys.oent2ent.resize(sys.nblksP);            // Only required for implementation No. 3 of BILU0
    sys.oent2entStart.resize(sys.nrowsP+1);     // Only required for implementation No. 3 of BILU0
    sys.LUoent2ent.resize(sys.nblksP);          // Only required for implementation No. 1 of BILU0
    sys.Loent2entStart.resize(sys.nrowsP+1);    // Only required for implementation No. 1 of BILU0
    sys.Uoent2entStart.resize(sys.nrowsP+1);    // Only required for implementation No. 1 of BILU0
    
    
    
    // Get length for elem and temp structures:
    nge = (nge >= ngeR) ? nge : ngeR;
    ngf = (ngf >= ngfR) ? ngf : ngfR;
    Int elemtempLen = max(cbsr_nblks, sys.nblksP) + cbsr_nrows + nge*ncd + nge*nd*nd + nge + nge*nd*nd + npf*nfe*ncd + 
...
//                 nqvR*ncu*nd*nc + nqvR + nqvR*nd*nd + nqvR*ncd + nqvR*nd*nd;
    ...
    sys.Mg.resize(blkSize*blkSize*sys.nblksP);
...
//     }
    ...
    if (sys.matvecImplementation == 1) {
        for (i = 0; i < sys.noThreads; i++)
            sys.xDenseRow[i].resize(blkSize*max(maxBlocksPerRow,maxBlocksPerRowP));
    }
    ...
    if (sys.precPrecision == 0) {
        if (sys.preconditioner == 0 || sys.preconditioner == 1 || sys.preconditioner == 2)
            sys.r_sp.resize(blkSize*max(sys.nrowsP,cbsr_nrows));                    // This is more memory than necessary for preconditioners 1 and 2
        if (sys.preconditioner == 0 || sys.preconditioner == 1)
            sys.rDenseRow_sp.resize(blkSize*max(maxBlocksPerRow,maxBlocksPerRowP));       // Only used in applyBILU0_sp
    ...
    Int start = 0;
    sys.C = &sys.v[start];
    start += max(cbsr_nblks, sys.nblksP);
    sys.w = &sys.v[start];
    start += cbsr_nrows;
...
//     temps.avfg_p1CG.resize(3*ngf*nfe);
    ...
    start += ngf*nd;
    temps.ftk = &sys.v[start];
    start += nge*max(npe,ndf)*ncu*ncu;
    temps.ftm = &sys.v[start];
    start += nge*max(npe,ndf)*ncu*ncu;
