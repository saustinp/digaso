...
void readmasterstruct(string filename, vector< masterstruct > &master, appstruct &app)
{
    ...
    for (i=0; i<nelem; i++) {        
        ...
        if ((dim==3) && (elemtype==1)) {
            master[elem].shapg1d.resize(dim);
            for (int i=0; i<dim; i++) 
                master[elem].shapg1d[i] = mkshapeg1d(master[elem].porder[i], master[elem].pgauss[i], nodetype);                        
        }
    }    
