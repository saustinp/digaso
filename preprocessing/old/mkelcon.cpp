...
{            
    ...
    for (i=0; i<nf; i++) {      
        ...
        // pv = dgnodes(elements[ei].permvertex,:,e1);
        // pv(elements[ei].face[if1],:);
        for (m=0; m<nvf; m++)             
            for (n=0; n<dim; n++)
                pf[n][m] = p[n*np+fi[m]];        
...
void mkelcon(vector<Int> &elcon,vector<double> &dgnodes, vector<elementstruct> elements, vector<double> &p, vector< vector< vector<double> > > &philocfc, 
        vector<Int> &f, vector<Int> &t2f, vector<Int> &elementtype, vector<Int> &isEDGface, Int dim, Int nvfmax, Int npfmax, Int nfemax)
{            
    Int i, j, k, m, n, e1, e2, ei, nvf, npe, nfe, npf, if1, if2, neg;
    Int nmax = nvfmax+3;
