...
void nbent(vector< vector<Int> > &nbrowent2ent, vector< vector<Int> > &nnrowent2ent, 
...
{    
    ...
    nnbsd =  (Int) dmd.nbsd.size();      
    nbrowent2ent.resize(nnbsd);
    nnrowent2ent.resize(nnbsd);
    nbentpart.resize(nnbsd);
    nnentpart.resize(nnbsd);    
    
    // allocate memory
    for (n=0; n<nbsd; n++) {        
        ng = nbmatrecv[n].size();    
        nbrowent2ent[n].resize(ng);
        nnrowent2ent[n].resize(ng);
        nnentpart[n].resize(ng);
        nk = 0;        
        for (j=0; j<ng; j++) {
            nn = dmd.bcrs_rowent2ent[mj+1] - dmd.bcrs_rowent2ent[mj];    
            nk += nn;
        }        
        nnentpart[n].resize(nk);
    }    

    for (n=0; n<nbsd; n++) { // for each neighboring cpu
        ng = nbmatrecv[n].size(); // number of global entities on neighboring cpu
        nk = 0;        
        for (j=0; j<ng; j++) { // for each global entity on neighboring cpu
            gj = nbmatrecv[j]; // global entity gj                              
            ...
            nbrowent2ent[n][j] = dmd.bcrs_rowent2ent[mj]; // row index of gj
            nnrowent2ent[n][j] = nn; // number of entities connected to gj               
            nnentpart[n][j] = nk; //summing numbers of entities                
            for (k=0; k<nn; k++) {
                ...
                nbentpart[n][nk+k] = dmd.entpart[cjk];                
            }              
            nk += nn;
        }
    }    
