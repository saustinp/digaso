...
{    
    if (app.nproc>1) { // parallel writing to the binary file   
        ...
        MPI_Comm_split(MPI_COMM_WORLD, (int) filenumber, (int) file_rank, &MPI_COMM_IO);
        ...
        std::string fileName = filePrefix; filePrefix is to be defined
        fileName += num2str(filenumber);
        
        MPI_File MPIfile;
        MPI_Status status;          
        MPI_File_open(MPI_COMM_IO, fileName, MPI_MODE_CREATE | MPI_MODE_WRONLY,
                          MPI_INFO_NULL, &MPIfile);  
        
        // Skip to the section of the file that this process will be writing.
        MPI_Offset fileOffset = sizeof(double) * meshsize[file_rank];
        MPI_File_set_view(MPIfile, meshsize[file_rank] (* sizeof(double)?) / fileOffset??, MPI_DOUBLE, MPI_DOUBLE,
                    "native", MPI_INFO_NULL);
        //MPI_File_seek(MPIfile, fileOffset, MPI_SEEK_SET);
        
        vector<double> temp(datasize,0);
        Int start = 0;
        for (Int i=0; i<N; i++)
            temp[start+i] = (double) mesh.ndims[i];
        start += N;
        for (Int i=0; i<ne; i++)
            temp[start+N] = (double) mesh.elementtype[i];
        start += ne;
        for (Int i=0; i<nve*ne; i++)
            temp[start+N] = (double) mesh.t[i];
        start += nve*ne;
        for (Int i=0; i<npe*ncd*ne; i++)
            temp[start+N] = mesh.dgnodes[i];
        start += npe*ncd*ne;
        for (Int i=0; i<npf*nfe*ne; i++)
            temp[start+N] = (double) mesh.elcon[i];
        start += npf*nfe*ne;
        for (Int i=0; i<nfe*ne; i++)
            temp[start+N] = (double) mesh.bf[i];
        start += nfe*ne;
        for (Int i=0; infe*ne; i++)
            temp[start+N] = (double) mesh.t2f[i];
        start += nfe*ne;
        for (Int i=0; i<nfe*ne; i++)
            temp[start+N] = (double) mesh.t2t[i];
        start += nfe*ne;
        for (Int i=0; i<(nvf+3)*nf; i++)
            temp[start+N] = (double) mesh.f[i];
        start += (nvf+3)*nf;
        for (Int i=0; i<nf; i++)
            temp[start+N] = (double) mesh.isEDGface[i];
        start += nf;
        for (Int i=0; i<nnbsd; i++)
            temp[start+N] = (double) mesh.nbsd[i];
        start += nnbsd;
        for (Int i=0; i<ne; i++)
            temp[start+N] = (double) mesh.extintelem[i];
        start += ne;
        for (Int i=0; i<ne; i++)
            temp[start+N] = (double) mesh.elem2cpu[i];
        start += ne;
        for (Int i=0; i<nf; i++)
            temp[start+N] = (double) mesh.extintface[i];
        start += nf;
        for (Int i=0; i<nf; i++)
            temp[start+N] = (double) mesh.face2cpu[i];
        start += nf;
        ...
        if (hybrid != 0) {
            ...
            start += ndh;
            for (Int i=0; i<ndh; i++)
                temp[start+N] = (double) mesh.ent2cpu[i];
            start += ndh;    
        }      
        
        // Write the process rank to the file:
        MPI_File_write(MPIfile, &temp[0], datasize, MPI_DOUBLE, &status);
...
{   
    if (app.nproc>1) { // parallel writing from the binary file       
        ...
        // Skip to the section of the file that this process will be writing.
        MPI_Offset fileOffset = sizeof(double) * solsize[file_rank];
        MPI_File_set_view(MPIfile, solsize[file_rank] (* sizeof(double)?) / fileOffset??, MPI_DOUBLE, MPI_DOUBLE,
                    "native", MPI_INFO_NULL);
        //MPI_File_seek(MPIfile, fileOffset, MPI_SEEK_SET);
        ...
        for (Int i=0; i<N; i++)
            temp[i] = sol.ndims[i];
        for (Int i=0; i<nudg; i++)
            temp[N+i] = sol.UDG[i];
        for (Int i=0; i<nuh; i++)
            temp[N+nudg+i] = sol.UH[i];
        for (Int i=0; i<npdg; i++)
            temp[N+nudg+nuh+i] = sol.PDG[i];
...
void writeOutput(appstruct &app, vector< masterstruct > &master, meshstruct &mesh, solstruct &sol, string fileout) 
{
